...
const localDirectory = path.resolve("public/downloads/pmd-borradores");
...
try {
  ...
  if (!resolvedRemoteDirectory.endsWith("/downloads/pmd-borradores")) {
    throw new Error(
      `La carpeta FTP resuelta no coincide: ${resolvedRemoteDirectory}`,
    );
  }
  await client.uploadFromDir(localDirectory);
  console.log(`Documentos base publicados en ${resolvedRemoteDirectory}`);
} finally {
  client.close();
}
